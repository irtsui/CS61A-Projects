...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    new_list = [[] for i in player_indices]
    for word_index in word_indices:
        times = time(match,0,word_index)
        players = 0
        for player_index in player_indices:
            if time(match,player_index,word_index) < times:
                times = time(match,player_index,word_index)
                players = player_index
        new_list[players] += [get_word(match,word_index)]
    return new_list
    # END PROBLEM 10
